...
main() {
  ...
  group('AuthBloc signInByEmailAndPassword', () {
    blocTest<AuthCubit, AuthState>(
      ...
      act: (cubit) {
        (authService as TestAuthService)
            .streamController
            .add(UserInfo(id: TestConsts.userId));
        cubit.setAppUser();
      },
      expect: () => [
        AuthState(
            status: AuthStatus.signedOut,
            user: UserInfo(id: TestConsts.userId)),
        AuthState(
    ...
    blocTest<AuthCubit, AuthState>(
      ...
      build: () {
        ...
        when(() => localStorageService.getSavedAppUser()).thenAnswer(
            (invocation) async => LocalUserInfo(userId: TestConsts.userId));
        return AuthCubitImpl(
            authService: authService,
            networkService: networkService,
            localStorageService: localStorageService,
      ...
      expect: () => [
        AuthState(
            ...
            user: UserInfo(id: TestConsts.userId)),
        AuthState(
            status: AuthStatus.signedIn, user: UserInfo(id: TestConsts.userId))
      ],
    );
  });
}
